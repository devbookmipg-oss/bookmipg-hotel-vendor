...
const Page = () => {
  ...
  const handleSave = async () => {
    if (editing) {
      ...
      SuccessToast('Inventory Purchase updated successfully');
    } else {
      await CreateNewData({
        auth,
        endPoint: 'inventory-purchases',
      ...
      SuccessToast('Inventory Purchase created successfully');
    }
    setFormOpen(false);
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    SuccessToast('Inventory Purchase deleted successfully');
    setDeleteOpen(false);
    setSelectedRow(null);
  };
    ...
    <>
      <Box sx={{ px: 3, py: 2, backgroundColor: '#efefef' }}>
        ...
        >
          ...
          <Typography color="text.primary">Inventory Purchase</Typography>
        </Breadcrumbs>
      </Box>
      {!data ? (
        <Loader />
